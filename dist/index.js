...
import { getClientHolding, getOpenAlgoOrders, getOpenOrders } from './account.js';
import { BASE_URL, BROKER_ID } from './config.js';
import { cancelAlgoOrder, cancelOrder, createAlgoOrder, createOrder } from './order.js';
import { getOrderbook } from './orderbook.js';
import { addAccessKey, registerAccount } from './register.js';
...
async function main() {
    ...
    await createOrder(symbol, OrderType.LIMIT, OrderSide.BUY, 
...
export function getAccountId(userAddress, brokerId) {
    ...
    return keccak256(abicoder.encode(['address', 'bytes32'], [userAddress, solidityPackedKeccak256(['string'], [brokerId])]));
}
main();
